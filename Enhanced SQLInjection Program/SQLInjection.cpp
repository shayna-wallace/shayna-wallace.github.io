...
// THE PURPOSE OF THIS PROGRAM IS DETECT SQL INJECTIONS AND PREVENT THEM FROM HAPPENING. ONCE THE SQL INJECTION
// HAS BEEN PROVENTED, IT SHOULD DISPLAY A MESSAGE. -SW 012422

//SQL INJECTION IS WHEN AN ATTACKER INSERTS MALICIOUS SQL CODE INTO A QUERY. -SW 012422
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
   ...
   // -SW 012422
  // USED ESCAPE ALL USER SUPPLIED INPUT METHOD
 // WITH THIS METHOD WHEN THE USER INPUTS DATA/CODE THE SYSTEM WILL ESCAPE THE USER INPUT BEFORE IT IS PUT INTO THE QUERY

 

    std::string sql1 = "SELECT * from USERS WHERE NAME = '* or 0=0'"; //Used escape all user input method for NAME data
    std::string sql2 = "SELECT * from USERS WHERE ID = '0=0'"; //Used escape all user input method for ID data
    std::string sql3 = "SELECT * from USERS WHERE PASSWORD = '?'"; //Used escape all user input method for PASSWORD data


    std::cout << "Suspected SQL Injection. SQL Injection FAILED. " << std::endl; //Created an error to print when SQL is detected
    return false;
